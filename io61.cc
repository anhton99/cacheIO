...
struct io61_file {
    ...
    // Single-slot cache
    off_t bufsize = BUFSIZE;        // Cache block size
    ...
    // Mutex 
    std::mutex m; 
    std::map<off_t, off_t> locked; // lock range
    std::condition_variable_any cv;

    // for mmap
    unsigned char *fdata;  // Pointer to mampped file
    size_t fsz;            // Size of  mmapped file
    size_t mmap_off;       // Offset for mmap
    bool is_mapped;  // Check if file is mapped 
...
io61_file* io61_fdopen(int fd, int mode) {
    ...
    f->mmap_off = 0;
    f->fsz = io61_filesize(f); 

    f->fdata = (f->fsz == (size_t) -1) ? (unsigned char *) MAP_FAILED 
        : (unsigned char *) mmap(nullptr, f->fsz, PROT_READ, MAP_SHARED, f->fd, 0);
    f->is_mapped = (f->fdata != MAP_FAILED);
    return f;
}
...
// may_overlap_with_other_lock(f, start, len)
...
bool may_overlap_with_other_lock(io61_file* f, off_t start, off_t len) {
    return f->locked.lower_bound(start) != f->locked.lower_bound(start + len);
}
...
ssize_t io61_read(io61_file* f, unsigned char* buf, size_t sz) {
    ...
    if (f->is_mapped) {
        ...
        if (sz + f->mmap_off > f->fsz) {
            sz = f->fsz - f->mmap_off;
        }
        memcpy(buf, &f->fdata[f->mmap_off], sz);
        f->mmap_off += sz;
        return sz;
    }
...
ssize_t io61_write(io61_file* f, const unsigned char* buf, size_t sz) {
    ...
    while (nwritten != sz) {
        ...
        size_t nleft = f->tag + f->bufsize - f->pos_tag;
...
int io61_seek(io61_file* f, off_t off) {
    ...
    if (f->mode == O_WRONLY) {
        ...
        // sets the file pointer to the byte offset off
        off_t seek_id = lseek(f->fd, off, SEEK_SET);
        if (seek_id == off) {
            f->pos_tag = off;
            f->end_tag = f->pos_tag;
            f->tag = f->pos_tag;
    ...
    int off_aligned = off - (off % BUFSIZE);
    if (f->tag <= off && off < f->end_tag) {
        // Requested offset is within buffered data range
        f->pos_tag = off;
        f->mmap_off = off;
        return 0;
    } 
    f->end_tag = f->pos_tag = f->tag = off_aligned;
    f->mmap_off = off;
    off_t id = lseek(f->fd, off_aligned, SEEK_SET);
    if (id == off_aligned) {
        io61_fill(f);  // Fill the buffer with data from the file
        f->pos_tag = off;
        f->mmap_off = off;
        f->positioned = false;
        return 0;
...
static int io61_pfill(io61_file* f, off_t off) {
    ...
    off = off - (off % 8192);
    ssize_t nr = pread(f->fd, f->cbuf, f->bufsize, off);
    if (nr == -1) {
        return -1;
    }
    f->tag = off;
...
int io61_try_lock(io61_file* f, off_t start, off_t len, int locktype) {
    ...
    f->locked.insert({start, start + len});
    return 0;
}
